...
#include <sys/poll.h>
#include "bionic/libc/kernel/common/linux/input.h"
#include <errno.h>
...
int main(int argc, char *argv[])
{
    ...
    while(1) {
        pollres = poll(ufds, nfds, -1);//poll函数用于监测多个等待事件，若事件未发生，进程睡眠，放弃CPU控制权，若监测的任何一个事件发生，poll将唤醒睡眠的进程，并判断是什么等待事件发生，执行相应的操作。poll函数退出后，struct pollfd变量的所有值被清零，需要重新设置。
        ...
        for(i = 1; i < nfds; i++) {
            if(ufds[i].revents) {
                if(ufds[i].revents & POLLIN) {
                    ...
                    //added by zhangxin11
                    fflush(stdout);//刷新缓冲区， 为IO重定向到文件所使用， 否则缓冲区4kb大小， 直到写满才会写文件
